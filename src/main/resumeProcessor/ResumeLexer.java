...
public class ResumeLexer implements Lexer {
	...
	public void lex() {
		...
		} else if (word.toUpperCase().matches("ACHIEVEMENT(S)?")) {
			token = new Token(word, TokenType.ACHIEVMENTS, true);
		} else if (word.equalsIgnoreCase("education") || word.equalsIgnoreCase("academic")
				|| word.equalsIgnoreCase("academics")) {
		...
		} else if (word.equalsIgnoreCase("skill") || word.equalsIgnoreCase("skills")
				|| word.equalsIgnoreCase("keywords")) {
			token = new Token(word, TokenType.SKILLS, true);
		} else if (word.equalsIgnoreCase("personal")) {

			token = new Token("personal", TokenType.PERSONAL, true);
		} else if (word.toLowerCase().matches("reference(s)?")) {
			token = new Token("references", TokenType.REFERENCES, true);
